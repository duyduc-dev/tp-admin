...
export function formatDate(
  ...
  format: string = FormatDate.DEFAULT,
) {
  if (!date) {
    return '';
  }
...
export const calculatePercent = (
  ...
  return hasPercentSymbol ? `${percent} %` : percent;
};

export const addMiddleEllipsis = (str: string): string => {
  if (str.length > 20) {
...
export function getYouTubeVideoCode(url: string) {
  try {
    ...
    // Extract the 'v' parameter which holds the video code
    const videoCode = urlObj.searchParams.get('v');

    if (videoCode) {
      return videoCode;
    } else {
      throw new Error('No video code found in the URL.');
    }
  } catch (error) {
